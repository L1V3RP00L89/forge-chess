...
| M4  | Coach-mode pedagogy pass — audit plain-language guidance against what an adult improver actually needs (plans/patterns over raw eval)        | **Shipped, one item open** — staged three-tier Coach reveal, `describeBestMove` gated behind it, journal takeaway count scaled to time control, and M8 reveal-event hook are all live; `describeBestMove`'s copy rewrite toward plans/patterns (unblocked 2026-08-18) is still outstanding | Renee → Priya → Dara |
...
**Still open:** `describeBestMove`'s one-sentence explanation is still generic move commentary, not plans/patterns framing — Renee's pushback flagged this as a deferred content pass "once the reveal ships," and the reveal shipped 2026-08-17. Also raised independently by Kris as backlog ("more human, plain-language AI coaching") and triaged into this milestone 2026-08-18 rather than treated as separate work. Owner: Renee (defines the rewrite) → Priya (implements). Not started.
