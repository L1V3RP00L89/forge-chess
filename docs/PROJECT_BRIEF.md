...
**Open item, unresolved:** the actual blocker to Coach-mode users seeing opening guidance — no token, no data — is still there. M6 shipped the presentation half (lead-with-one-move, disclosure, Coach-mode line) but not the friction fix Renee flagged as the highest-value change. Needs a real decision from Kris/Owen: prompt Coach-mode users to add a token (reintroduces the friction Renee wanted to avoid), find/use a different data source that doesn't require auth, or accept that Coach mode's opening line stays opening-name-only until a token is set in Pro mode first.
