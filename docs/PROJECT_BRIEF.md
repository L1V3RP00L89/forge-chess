...
| M6  | Opening explorer presentation — thin, memorable repertoire framing vs. theory-dump                                                           | **Scoped 2026-08-19 — Renee + Dara passes done, ready for Priya**                                      | Renee → Dara → Priya |
...
**Dara's layout pass (2026-08-19):** full spec in `User Inbox/Opening Intel — Coach Surfacing & Pro Reflow Layout Spec.md`, addressed to Priya. Key findings beyond layout: traced the token-friction question to `useOpeningExplorer.ts:84` (`if (!hasAuth) return`) — no explorer request fires at all without a token, for either source, which is a client-side restriction, not a Lichess API requirement (their public Explorer API allows unauthenticated Masters/Lichess reads at a lower rate limit). Recommends narrowing the token gate to Lichess-source/rating-filtered requests only, leaving Masters-source (including the new Coach-mode line) token-free. Coach-mode line specced as one unbordered text row reusing `useOpening`'s existing offline FEN lookup plus a single Masters-source move, no card chrome, no click target. Pro mode specced to lead with the top move as its own row, with the remaining list moved behind a `<details>` disclosure reusing the pattern already shipped for the Review tab (`bd92c40`). Open item back to Priya: confirm the anonymous Masters rate limit holds up before wiring the Coach-mode line live.
